...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    class PDF extends FPDF {
        function Header() {
            // Logo en marca de agua (centrado, transparente)
            $this->Image('../imagenes/logo_perutec.png', 40, 70, 130, 130);
        }
    }
    ...
    $pdf->Cell(0,10,utf8_decode('RECIBO DE VENTA'),0,1,'R');
    ...
    $pdf->Cell(30,8,utf8_decode('DÍA'),1,0,'C',true);
    $pdf->Cell(30,8,utf8_decode('MES'),1,0,'C',true);
    $pdf->Cell(30,8,utf8_decode('AÑO'),1,1,'C',true);
    ...
    $pdf->MultiCell(0,6,utf8_decode("Venta de artículos tecnológicos, estudio fotográfico, revelados de fotos,\nampliaciones, carnet, pasaporte, visa, cuadros, ediciones y montajes, copias e impresiones."),0,'L');

    // ==== CLIENTE ====
    $pdf->Ln(4);
    $pdf->SetFont('Arial','B',11);
    $pdf->Cell(0,8,utf8_decode("Cliente: $nombre_cliente"),0,1);
    $pdf->SetFont('Arial','',11);
    $pdf->Cell(0,8,utf8_decode("Celular: $n_celular_cliente"),0,1);
    $pdf->Cell(0,8,utf8_decode("Vendedor: $nombre_vendedor"),0,1);
    $pdf->Ln(3);
    ...
    $pdf->Cell(30,8,utf8_decode('CANT.'),1,0,'C',true);
    $pdf->Cell(90,8,utf8_decode('DESCRIPCIÓN'),1,0,'C',true);
    $pdf->Cell(35,8,utf8_decode('P.UNIT'),1,0,'C',true);
    ...
    foreach ($carrito as $item) {
        ...
        $pdf->Cell(35,8,'S/ '.number_format($precio_unitario,2),1,0,'R');
        $pdf->Cell(35,8,'S/ '.number_format($subtotal,2),1,1,'R');
    }
    ...
    $pdf->SetFillColor(200,220,255);
    $pdf->Cell(155,10,utf8_decode('TOTAL A PAGAR'),1,0,'R',true);
    $pdf->Cell(35,10,'S/ '.number_format($total_final,2),1,1,'R',true);
    ...
    // ==== REDIRECCIÓN ====
    header("Location: ../administrador/venta_exitosa.php?id=$id_venta");
    exit();
}
?>
